...
import com.myteldemo.demo.message.ResponseMessage;
...
public class FilesController {
    ...
    @GetMapping("/files")
    public ResponseEntity<List<Employee>> getListFiles() {
        List<Employee> fileInfos = storageService.loadAll().map(path -> {
            String filename = path.getFileName().toString();
            String url = MvcUriComponentsBuilder
                    .fromMethodName(FilesController.class, "getFile", path.getFileName().toString()).build().toString();

            return new Employee(filename, url);
